...
import { Play, ChevronRight, Flame, User, Diamond, Facebook, Instagram, Twitter, Linkedin } from "lucide-react";
...
function Header() {
  return (
    <header className="sticky top-0 z-50 backdrop-blur-xl bg-background/85 border-b border-border">
      <div className="mx-auto max-w-[1600px] px-4 sm:px-6 lg:px-10 h-14 flex items-center gap-4">
        ...
        <div className="hidden sm:flex items-center gap-3 text-sm">
          <div className="flex items-center gap-1.5 font-semibold text-streak">
            <Flame className="size-4 fill-current" />
            <span>477 day streak</span>
          </div>
          <div className="flex items-center gap-1 font-semibold text-primary">
            <Diamond className="size-4 fill-current" />
            <span>Top Reviewer</span>
          </div>
        </div>
        <span className="text-sm font-semibold text-money tabular-nums">$3,460.53</span>
        <button aria-label="Profile" className="size-8 rounded-full bg-secondary border border-border flex items-center justify-center hover:bg-muted transition">
...
function PosterCard({ item }: { item: Item }) {
  return (
    <div className="group poster aspect-[2/3] relative overflow-hidden">
      <img src={item.src} alt={item.title} loading="lazy" />
      <div className="absolute inset-0 z-20 flex flex-col items-center justify-center text-center bg-black/80 p-3 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
        ...
        <button className="rounded-full bg-primary text-primary-foreground px-4 py-2 text-xs sm:text-sm font-semibold hover:scale-105 transition-transform shadow-lg">
          Start Reviewer
        </button>
